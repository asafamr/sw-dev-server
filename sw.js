(() => {
  ...
  var sw = self;
  var store = import_localforage.default.createInstance({
    name: "sw-dev-server"
  });
  sw.addEventListener("fetch", function(event) {
    ...
    if (!new URL(event.request.url).pathname.startsWith(base + "/dev"))
      ...
      return event.respondWith(Promise.resolve().then(async () => {
        try {
          const got = await store.getItem(event.request.url);
  ...
  sw.addEventListener("message", async (msg) => {
    if (msg?.data?.type === "add") {
      ...
      await store.setItem(new URL(url, msg.source.url).toString(), { content, mime });
